...
__global__ void __launch_bounds__(blockSize) blockReduceAdd(T* a, std::size_t numElement, T* b) {
    ...
    T res;
    if constexpr (opType == OpType::Add) {
        res = alyx::blockReduce(val, init, [](T a, T b) { return a + b; });
    } else if constexpr (opType == OpType::Multiply) {
        res = alyx::blockReduce(val, init, [](T a, T b) { return a * b; });
    } else if constexpr (opType == OpType::Max) {
        res = alyx::blockReduce(val, init, [](T a, T b) { return max(a, b); });
    } else if constexpr (opType == OpType::Min) {
        res = alyx::blockReduce(val, init, [](T a, T b) { return min(a, b); });
    }
...
public:
    void run() {
        ...
        blockReduceAdd<blockSize, T, opType><<<gridSize, blockSize>>>(ad, numElement, bd);
